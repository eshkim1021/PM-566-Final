...
```{r Create Figure for Age and COVID-19 Death, echo = FALSE}
Age %>% 
...
```{r, Gender and COVID-19, echo = FALSE}
gender_death %>% 
...
```{r Figure of Covid-19 Death by Age and Gender, echo = FALSE, width = 1.5}
q1 %>% 
  filter(!(Age_Group %in% "5-14 years")) %>%
  filter((State %in% "United")) %>%
   filter(!(Gender %in% "All Sexes")) %>% 
...
```{r Figure of Age with Pnemonia and COVID Death, echo= FALSE}
q2 %>% 
  filter(!(Age_Group %in% "5-14 years")) %>% 
...
```{r Figure of Proportoinal Deaths by Covid-19, echo = FALSE}
